...
const carStop = async (event: MouseEvent) => {
    ...
    const sectionPage = sectionCar.parentElement as HTMLElement;
    const nameCar = (sectionCar.querySelector('.wrapper-buttons-select-remove__h4') as HTMLSpanElement).innerText;
    if (sectionPage.getAttribute('winner') === nameCar) {
        sectionPage.removeAttribute('winner');
    }
};
